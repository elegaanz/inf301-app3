...
int main(int argc, char **argv) {
        ...
        if (!possible) {
            ...
            fprintf(
                ...
                "\033[0;31mERREUR\033[0m %s n'a pas été insérée dans l'arbre %s\
					(ajouter_carac a renvoyé -1) mais elle aurait pourtant du être ajoutée\n",
                nom_carac, nom_fichier);
            return -1;
        }
    }
    ...
    // Verifier l'arbre
    return verifier_arbre(a, nom_fichier, nb_especes, nb_carac,
                          nb_especes_a_tester, especes_caracs);
}
