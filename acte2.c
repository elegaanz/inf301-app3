...
int main(int argc, char **argv) {
    ...
    if (!present) {
        ...
        if (rechercher_espece(a, nom_espece, &seq) == 0)
            fprintf(stderr,
                    "\033[0;31mERREUR\033[0m sur %s\n\t %s est absent de "
                    "l'arbre %s "
                    "mais rechercher_espece a renvoyé présent\n",
        ...
        if (getline(&line, &len, f) != -1) {
            ...
            fprintf(stderr,
                    "Erreur : pas de nombre d'espece dans le fichier test\n");
            return 1;
        }
        ...
        if (present != 0) {
            fprintf(stderr,
                    "\033[0;31mERREUR\033[0m sur %s\n\tL'espece %s n'a pas été "
                    "trouvée dans l'arbre %s \n",
                    fichier_test, nom_espece, nom_fichier);
            return 1;
        }
        return test_list_carac(expected_nb_carac, expected_caracs, seq);
    }
    return 0;
}
